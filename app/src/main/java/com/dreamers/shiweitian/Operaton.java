...
public class Operaton
{
	...
	public String userInfo(String url, String id)
	{   // User user=(User)getApplication();
		String result = null;
		ConnNet connNet=new ConnNet();
		List<NameValuePair> params=new ArrayList<NameValuePair>();
		params.add(new BasicNameValuePair("method", "getUserById"));
	...
	public String upload(String url, String shopId, String userId, String content)
	{
		// User user=(User)getApplication();
		...
		report.setTime(time.year+"年"+time.month+"月"+time.monthDay+"日");
		report.setState("已提交");
		List<Report> reports = new LinkedList<Report>();
		reports.add(report);
		String reportJson = new JsonUtil<Report>().getJsonData(reports);
		String result = null;
		ConnNet connNet=new ConnNet();
		List<NameValuePair> params=new ArrayList<NameValuePair>();
		params.add(new BasicNameValuePair("method", "addReport"));
		params.add(new BasicNameValuePair("shopId", shopId));
		params.add(new BasicNameValuePair("userId", userId));
		params.add(new BasicNameValuePair("reportJson", reportJson));
		try {
			HttpEntity entity=new UrlEncodedFormEntity(params, HTTP.UTF_8);
			HttpPost httpPost=connNet.gethttPost(url);
			System.out.println(httpPost.toString());
